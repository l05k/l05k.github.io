...
和 Obsidian 完全相同的写法（`$...$` 行内、`$$...$$` 块级）。构建时用 MathJax 服务端渲染成 CHTML 文本（与 Obsidian 同一引擎），**读者打开页面时不需要下载任何公式引擎，公式是真实文本、可以选中**：
...
**复制公式为 Markdown**：选中包含公式的文字后复制（可以只选公式，也可以连同上下文一起划选），剪贴板中公式会以 Markdown 源码呈现 —— 行内公式 `$...$`、块级公式 `$$...$$`，周围文字原样保留；纯文字选区不受影响（走原生复制）。悬停公式可见源码提示。这样读者可以把公式连同上下文直接搬回自己的笔记 / Obsidian 里。
